...
from valkka.discovery.wsdiscovery.udp import UDPMessage
from valkka.discovery.wsdiscovery.envelope import SoapEnvelope
from valkka.discovery.wsdiscovery.actions import *
from valkka.discovery.wsdiscovery.uri import URI
from valkka.discovery.wsdiscovery.util import _getNetworkAddrs, matchScope
from valkka.discovery.wsdiscovery.util import _generateInstanceId, extractSoapUdpAddressFromURI
from valkka.discovery.wsdiscovery.message import createSOAPMessage, parseSOAPMessage
from valkka.discovery.wsdiscovery.service import Service
from valkka.discovery.wsdiscovery.namespaces import NS_D
...
MULTICAST_PORT = 3702
MULTICAST_IPV4_ADDRESS = "172.21.104.100" #"239.255.255.250"
...
class NetworkingThread(_StoppableDaemonThread):
    ...
    def addSourceAddr(self, addr):
        ...
        except socket.error as e:  # if 1 interface has more than 1 address, exception is raised for the second
            pass

        sock = self._createMulticastOutSocket(addr, self._observer.ttl)
        self._multiOutUniInSockets[addr] = sock
        # print('_multiOutUniInSockets = ', self._multiOutUniInSockets)
    ...
    def _recvMessages(self):
        for fd, event in self._poll.poll(0):
            ...
            if env is None: # fault or failed to parse
                continue
            # print('======================= ', "addr = ", addr, ", fd = ", fd, ", data = ", data)
    ...
    def _sendMsg(self, msg):
        ...
        if msg.msgType() == UDPMessage.UNICAST:
            # print('msg addr = ', msg.getAddr(), ", msg port = ", msg.getPort())
            ...
            for sock in list(self._multiOutUniInSockets.values()):
                # print('msg addr = ', msg.getAddr(), ", msg port = ", msg.getPort(), ", sock = ", sock)
    ...
    def _sendPendingMessages(self):
        ...
        # print('queue len = ', len(self._queue))
        ...
        if msg.canSend():
            ...
            if not (msg.isFinished()):
                # print('msg is not finished')
                self._queue.append(msg)
        else:
            # print('msg can not send, re append to queue')
            self._queue.append(msg)
            time.sleep(0.01)
...
class WSDiscovery:
    ...
    def searchServiceInRange(self, ip_range: list, types=None, scopes=None, timeout=3):
        services = []
        for ip in ip_range:
            global MULTICAST_IPV4_ADDRESS
            MULTICAST_IPV4_ADDRESS = ip
            print("============= Start Search Services In IP ", MULTICAST_IPV4_ADDRESS, " ===============")
            service = self.searchServices(types, scopes, timeout)
